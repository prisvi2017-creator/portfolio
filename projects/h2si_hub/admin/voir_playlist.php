...
if(isset($_POST['supprimer_pdf'])){
  ...
  if($verify_pdf->rowCount() > 0){
     ...
     unlink('dossierpdf/'.$fetch_video['pdf']);
     $delete_cours = $con->prepare("DELETE FROM t_cours WHERE id = :id");
     $delete_cours->bindParam(":id", $delete_id);
     $delete_cours->execute();
     $success_msg[] = 'Cours supprimé avec succès!';
...
<div class="menu">
   ...
   $select_playlist = $con->prepare("SELECT * FROM t_playlist WHERE id = :id AND id_enseignant = :id_enseignant");
...
<h1 class="heading">playlist cours</h1>


<div class="box-container">
<?php

      $select_videos = $con->prepare("SELECT * FROM t_cours WHERE id_enseignant = :id_enseignant AND id_playlist = :id_playlist");
      $select_videos->bindParam(":id_enseignant",$id_enseignant);
      $select_videos->bindParam(":id_playlist", $playlist_id);
   ...
   <div class="box">
      ...
      <img src="dossierimage/<?= $fetch_videos['image']; ?>" class="thumb" alt="">
      <h3 class="title"><?= $fetch_videos['titre']; ?></h3>
      <form action="" method="post" class="flex-btn">
         <input type="hidden" name="video_id" value="<?= $video_id; ?>">
      
         <?php
            // Vérification si le cours est une vidéo
            if (!empty($fetch_videos['video'])) {
            ?>
         <a href="modifier_cours.php?type=video&get_id=<?= $video_id; ?>" class="inline-option-btn">Modifier</a>
         <input type="submit" value="Supprimer" class="delete-btn" onclick="return confirm('supprimer ce cours?');" name="supprimer">
         <?php
            // Vérification si le cours est un PDF
            } elseif (!empty($fetch_videos['pdf'])) {
            ?>
...
?>
   ...
   <?php
         ...
         echo '<p class="empty">aucun cours disponible! <a href="ajouter_cours.php" class="btn" style="margin-top: 1.5rem;">ajouter cours</a></p>';
      }
   ?>
</div>
