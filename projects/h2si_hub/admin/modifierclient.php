...
$id = $_GET["id"];
include("connexion.php");
$sql2 = "SELECT* FROM t_client WHERE id='$id'";
foreach($con->query($sql2)as $result2){
   }
	?>

  <form  action="updateclient.php" method="post">
  <input type="hidden" name="id_client" value="<?php echo $id?>" >
	<input  style="margin-top: 30px;" type="text" placeholder=" <?php echo $result2[1];?>" name="nom_client"  class="box"  pattern = "[A-Za-zÀ-ÿ\s']*[^0-9]+">
    <input  style="margin-top: 30px;" type="text" placeholder=" <?php echo $result2[2];?>" name="prenom_client"  class="box"  pattern = "[A-Za-zÀ-ÿ\s']*[^0-9]+">
    <input  style="margin-top: 30px;" type="email" placeholder=" <?php echo $result2[3];?>" name="mail_client"  class="box">
   <input style="margin-top: 50px;" type="submit" name="submit"  value="Modifier" class="btn">
 <a href="client.php"><button class="btn-rt">Retour</a></button>
  </form>
