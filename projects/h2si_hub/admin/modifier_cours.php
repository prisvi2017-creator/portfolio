...
if(isset($_POST['submit'])){
 ...
 $update_cours->bindParam(":id", $video_id);
 ...
 if(!empty($playlist)){
    ...
    $update_playlist->bindParam(":id", $video_id);
 ...
 if(!empty($image)){
    if($image_size > 2000000){
       ...
       $update_image->bindParam(":id", $video_id);
...
$ancien_video = $_POST['ancien_video'];
$video = $_FILES['video']['name'];
$video_tmp_name = $_FILES['video']['tmp_name'];
$video_folder = 'dossiervideo/'.$video;

if(!empty($video)){
   $update_video = $con->prepare("UPDATE t_cours SET video = :video WHERE id = :id");
   $update_video->bindParam(":video", $video);
   $update_video->bindParam(":id", $video_id);
   move_uploaded_file($video_tmp_name, $video_folder);
   if($ancien_video != '' AND $ancien_video != $video){
      unlink('dossiervideo/'.$ancien_video);
   }
}
...
<?php
         $select_videos = $con->prepare("SELECT * FROM t_cours WHERE id = :id AND id_enseignant= :id_enseignant");
         $select_videos->bindParam(":id", $get_id);
...
<input type="hidden" name="video_id" value="<?= $fetch_videos['id']; ?>">
  <input type="hidden" name="ancien_video" value="<?= $fetch_videos['video']; ?>">
<input type="hidden" name="ancien_image" value="<?= $fetch_videos['image']; ?>">
<p>Titre <span>*</span></p>
   <input type="text" name="titre" maxlength="100" required placeholder="entrer un titre" value="<?= $fetch_videos['titre']; ?>" class="box"> 
   <p>description <span>*</span></p>
   <textarea name="description" class="box" required placeholder="écrivez une description" maxlength="1000" cols="30" rows="10"><?= $fetch_videos['description']; ?></textarea>
   <p>Modifier la miniature<span>*</span></p>
   <div class="thumb">
         <img src="dossierimage/<?= $fetch_videos['image']; ?>" alt="">
      </div>
   <input class="box" type="file" name="file" accept="image/*" >
   <video src="dossiervideo/<?= $fetch_videos['video']; ?>" controls></video>
...
?>
   ...
   <div class="flex-btn">
         <a href="voir_cours.php?get_id=<?= $video_id; ?>" class="option-btne">voir le cours</a>
      </div>   
</form>
<?php
      } 
