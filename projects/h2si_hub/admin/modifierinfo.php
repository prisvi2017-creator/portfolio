...
$id = $_GET["modifie"];
include("connexion.php");
$select_info = $con->prepare("SELECT * FROM t_info WHERE id = :id ");
$select_info->bindParam(":id", $id);
$select_info->execute();
if($select_info->rowCount() > 0){
while($fetch_info = $select_info->fetch(PDO::FETCH_ASSOC)){
   $id_info = $fetch_info['id'];

	?>
  
    <form  action="updateinfo.php" method="post">
    <input type=text name="id" value="<?=$id_info; ?>" >
    <input  style="margin-top: 30px;" type="text" name="titre" placeholder="<?=$fetch_info['titre']; ?>" class="box">
    <textarea style="margin-top: 30px;"  name="info" placeholder="<?=$fetch_info['info']; ?>"  id="" cols="30" rows="10"></textarea>
   <input style="margin-top: 50px;" type="submit" name="submit"  value="Ajouter" class="btn">
   <a href="newsletter.php"><button class="btn-rt">Retour</a></button>
	</form>
    <?php
      } 
   }else{
      echo '<p class="empty">impossible!</p>';
   }
   ?>
    </div>
