...
if(isset($_POST['supprimer'])){
   ...
   $delete_video_thumb = $con->prepare("SELECT image FROM t_cours WHERE id = :id LIMIT 1");
   $delete_video_thumb->bindParam(":id", $delete_id);
   $delete_video_thumb->execute();
   $fetch_thumb = $delete_video_thumb->fetch(PDO::FETCH_ASSOC);
   unlink('dossierimage/'.$fetch_thumb['image']);

   $delete_video = $con->prepare("SELECT video FROM t_cours WHERE id = :id LIMIT 1");
...
<div class="menu">
   ...
   ?>
<div class="container">
   <video src="dossiervideo/<?= $fetch_content['video']; ?>" autoplay controls poster="dossierimage/<?= $fetch_content['image']; ?>" class="video"></video>
   ...
   <form action="" method="post">
      <div class="flex-btn">
         ...
         <a href="modifier_cours.php?get_id=<?= $video_id; ?>" class="inline-option-btn">Modifier</a>
         <input type="submit" value="Supprimer" class="delete-btn" onclick="return confirm('supprimer la video?');" name="supprimer">
      </div>
   </form>
</div>
<?php
