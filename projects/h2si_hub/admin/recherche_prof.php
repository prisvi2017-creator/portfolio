<?php
  ...
  if(isset($_POST['supprimer_playlist'])){
...
if(isset($_POST['search']) or isset($_POST['search_btn'])){
    $search = $_POST['search'];
      $select_videos = $con->prepare("SELECT * FROM t_cours WHERE titre LIKE '%{$search}%' AND id_enseignant = :id_enseignant ORDER BY date DESC");
...
?>
   ...
   <?php
       if(isset($_POST['search']) or isset($_POST['search_btn'])){
        $search = $_POST['search'];
         $select_playlist = $con->prepare("SELECT * FROM t_playlist WHERE titre LIKE '%{$search}%' AND id_enseignant = :id_enseignant ORDER BY date DESC");
      ...
      <div class="box">
         ...
         <form action="" method="post" class="flex-btn">
            ...
            <input type="submit" value="supprimer" class="delete-btn" onclick="return confirm('supprimer cette playlist?');" name="supprimer_playlist">
         </form>
      ...
      <?php
           ...
           echo '<p class="empty">Aucune playlist trouvé!</p>';
        }}else{
           echo '<p class="empty">Chercher quelque chose!</p>';
        }
      ?>
