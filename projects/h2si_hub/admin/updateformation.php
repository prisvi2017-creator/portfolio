...
<div class="menu">
 ...
 if(!empty($nom_form)){
     $update_nom = $con->prepare("UPDATE t_formation SET nom_form =:nom_form WHERE id = :id");
     $update_nom ->bindParam(":nom_form", $nom_form);
     $update_nom ->bindParam(":id", $id);
     $update_nom ->execute();
    $success_msg[] = 'formation modifié avec succès!';
  }
...
?>
   ...
   "><?php echo $fetch_enseignant['nom_prof']. ' ' . $fetch_enseignant['prenom_prof']; ?></p></td>
       ...
       <td class='btn-mod'><a href='voirplaylist.php'>Voir playlist</a></td>
