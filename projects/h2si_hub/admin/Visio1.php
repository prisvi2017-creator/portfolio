<?php
 ...
 if (isset($_POST["titre"]) && isset($_POST["formation"]) && !empty($_POST["titre"]) && !empty($_POST["formation"])) {
   $titre = $_POST["titre"];
   $formation = $_POST["formation"];
   
   // Générer un lien de réunion unique
   $roomName = "MeetingRoom" . uniqid();
   $meetingLink = "https://meet.jit.si/" . $roomName;

   include("connexion.php");
   // Enregistrer les détails de la réunion dans la base de données
   $sql = "INSERT INTO t_visioconference (id_enseignant, titre, lien, id_formation) VALUES (:id_enseignant, :titre, :lien, :id_formation)";
   $stmt = $con->prepare($sql);
   $stmt->bindParam(":id_enseignant", $id_enseignant);
   $stmt->bindParam(":titre", $titre);
   $stmt->bindParam(":lien", $meetingLink);
   $stmt->bindParam(":id_formation", $formation);
   $stmt->execute();

   $success_msg[] = 'Lien de visioconférence envoyé avec succès!';
}
  ?>
...
<div class="menu">
   ...
   <section class="video-form">
    <h1 class="heading">Initier une visioconférence</h1>

    <div class="box-container">
        <form action="" method="post">
            <p>Titre de la réunion <span>*</span></p>
            ...
            <input type="submit" value="Démarrer la visioconférence" class="btn">
            <a href="voir_visio.php" class="btn" >Voir la reunion</a>
